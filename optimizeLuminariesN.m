...
for ii=1:cantRepeat
    ...
    MENOR=Inf; %%% Esta variable contiene el mejor valor de la función a optimizar (hasta el momento).%76;
...
%                 % Penalty
%                 c1 = -1*((max(restrictionObj.maxPower - sum(luminarieObj.luminairePower(selectedLuminary)), 0)^2)*100);
%                 if (c1 < 0)
%                     c1 = c1 + luminarieObj.maxPower * problemObj.lumCount; % c1 >= luminarieObj.maxPower
%                 end
%                 objetivo = -1*(c1 + objetivo);
                %---------------------------------------
